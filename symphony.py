    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await dedupe_queue_table_by_track_key(cur, "symphony_queue", bot_name="symphony")
                ...
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    for g in guilds:
                        ...
                        await cur.execute(
                            ...
                            LEFT JOIN symphony_playback_state ps
                              ON ps.guild_id = seed.guild_id AND ps.bot_name = 'symphony'
                            LIMIT 1
                            """,
                            (g.id, g.id, g.id),
                        )
...
async def restore_queue_from_backup(cur, guild_id, requester_id=None):
    ...
    removed_content_dupes = await dedupe_queue_table_by_track_key(cur, "symphony_queue_backup", guild_id=guild_id, bot_name="symphony")
...
async def filter_cmd(interaction: discord.Interaction, mode: str):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                if mode != 'none': await cur.execute("UPDATE symphony_guild_settings SET filter_mode = %s, custom_speed = 1.0, custom_pitch = 1.0, custom_modifiers_left = 0 WHERE guild_id = %s", (mode, interaction.guild.id))
                else: await cur.execute("UPDATE symphony_guild_settings SET filter_mode = %s, custom_speed = 1.0, custom_pitch = 1.0, custom_modifiers_left = 0 WHERE guild_id = %s", (mode, interaction.guild.id))
    new_speed = 1.0
    if interaction.guild.voice_client:
        wav_filters = wavelink.Filters()
