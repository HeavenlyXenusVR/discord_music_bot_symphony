...
async def snapshot_queue_backup(cur, guild_id):
    ...
    if insert_data:
        await cur.executemany(
            "INSERT INTO symphony_queue_backup (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)",
            insert_data,
        )
...
async def delete_live_queue_copies(cur, guild_id, video_url, title):
    ...
    if not target_key:
        return 0
    await cur.execute(
        "SELECT id, video_url, title FROM symphony_queue WHERE guild_id = %s AND bot_name = 'symphony'",
        (guild_id,),
    )
    rows = list(await cur.fetchall() or [])
    deleted = 0
...
async def shuffle_queue_rows(cur, guild_id, *, preserve_first=True):
    ...
    try:
        ...
        if insert_data:
            await cur.executemany(
                "INSERT INTO symphony_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)",
                insert_data,
            )
...
async def restore_queue_from_backup(cur, guild_id, requester_id=None):
    ...
    for row in rows:
        ...
        insert_data.append((guild_id, 'symphony', url, title, requester_id if requester_id is not None else backup_requester_id))
    if insert_data:
        await cur.executemany(
            "INSERT INTO symphony_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)",
            insert_data,
...
async def restore_active_playback_entry(cur, guild_id, requester_id=None):
    ...
    await cur.execute(
        "SELECT video_url, title FROM symphony_queue WHERE guild_id = %s AND bot_name = 'symphony' ORDER BY id ASC LIMIT 25",
        (guild_id,),
    )
...
async def requeue_failed_track(cur, guild_id, channel_id, url, title, requester_id, *, position=0, reason="recovery"):
    ...
    async with get_track_requeue_lock(guild_id):
        ...
        if recent_until and now < recent_until:
            await cur.execute(
                "SELECT video_url, title FROM symphony_queue WHERE guild_id = %s AND bot_name = 'symphony' ORDER BY id ASC LIMIT 12",
                (guild_id,),
            )
            existing_rows = list(await cur.fetchall() or [])
            already_queued = any(
        ...
        await cur.execute(
            "SELECT video_url, title FROM symphony_queue WHERE guild_id = %s AND bot_name = 'symphony' ORDER BY id ASC LIMIT 8",
            (guild_id,),
        )
        existing_rows = list(await cur.fetchall() or [])
        already_queued = any(
                    ...
                    async with conn.cursor() as cur:
                        ...
                        if reason == "FINISHED":
                            ...
                            if loop_mode == 'queue':
                                ...
                                await insert_queue_front(cur, "symphony_queue", guild.id, "symphony", track_uri, track_title, original_requester)
                            else:
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if not next_song:
                    await cur.execute(
                        "UPDATE symphony_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0, is_playing = FALSE, is_paused = FALSE WHERE guild_id = %s AND bot_name = 'symphony'",
                        (guild.id,),
                    )
...
async def stop_playback(guild):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("UPDATE symphony_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0, is_playing = FALSE, is_paused = FALSE WHERE guild_id = %s AND bot_name = 'symphony'", (guild.id,))
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if queue_rows:
                    await cur.executemany(
                        "INSERT INTO symphony_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)",
                        [(interaction.guild.id, 'symphony', url, title, requester_id) for url, title, requester_id in queue_rows],
                    )
                    try:
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                insert_data = [(interaction.guild.id, 'symphony', track.uri, track.title, interaction.user.id)]
                insert_data.extend((
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("UPDATE symphony_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0, is_playing = FALSE, is_paused = FALSE WHERE guild_id = %s AND bot_name = 'symphony'", (interaction.guild.id,))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT id, video_url, title FROM symphony_queue WHERE guild_id = %s AND bot_name = 'symphony' ORDER BY id ASC LIMIT 1 OFFSET %s", (interaction.guild.id, index-1))
                row = await cur.fetchone()
                if row:
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT video_url, title FROM symphony_queue WHERE guild_id = %s AND bot_name = 'symphony' ORDER BY id ASC LIMIT %s", (interaction.guild.id, skip_count))
                rows = list(await cur.fetchall() or [])
                if rows:
                    try:
                        await cur.execute("START TRANSACTION")
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if snapshot_rows:
                    ...
                    await cur.executemany(
                        "INSERT INTO symphony_active_playlist_tracks (guild_id, bot_name, playlist_url, position_idx, track_key, video_url, title, requester_id) VALUES (%s, 'symphony', %s, %s, %s, %s, %s, %s)",
    ...
    for (guild_id, url, known_count, req_id, channel_id), data in zip(playlists, results):
        try:
            if isinstance(data, Exception):
                logger.error("[symphony] Playlist sync extraction failed for guild %s: %s", guild_id, data)
                continue
            ...
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        ...
                        await cur.execute("SELECT track_key, video_url, title, requester_id FROM symphony_active_playlist_tracks WHERE guild_id = %s AND bot_name = 'symphony' ORDER BY position_idx ASC", (guild_id,))
                        ...
                        purged_backup = 0
                        if removed_counts:
                            await cur.execute("SELECT id, video_url, title FROM symphony_queue WHERE guild_id = %s AND bot_name = 'symphony' ORDER BY id ASC", (guild_id,))
                            ...
                            await cur.execute("SELECT id, video_url, title FROM symphony_queue_backup WHERE guild_id = %s AND bot_name = 'symphony' ORDER BY id ASC", (guild_id,))
                        ...
                        if added_rows:
                            await cur.executemany(
                                "INSERT INTO symphony_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)",
                                [(guild_id, 'symphony', t_url, t_title, t_req) for t_url, t_title, t_req, _key in added_rows],
                            )
                            try:
            ...
            logger.error("[symphony] Playlist sync loop failed for guild %s: %s", guild_id, e, exc_info=True)
...
async def queue_integrity_check_loop():
    ...
    from symphony_queue_backup so playback can resume cleanly.
    """
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    for row in candidates:
                        ...
                        await cur.execute(
                            "SELECT video_url, title, requester_id FROM symphony_queue_backup "
                            "WHERE guild_id = %s AND bot_name = 'symphony' "
