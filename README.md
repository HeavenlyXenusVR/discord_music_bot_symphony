...
![Symphony bot icon](assets/bot-icon.png)

Symphony is one node in the 13-bot Discord music fleet. The fleet's biggest-sounding node, tuned for full, layered playback. It owns its queue tables, reports playback state to the shared swarm database, and is controlled directly from Discord, Aria, and SwarmPanel.

## Identity

- Discord display name: **GWS Music Bot (Symphony)**
- Internal node name: `symphony`
- Command namespace: `/symphony_main_*`
...
All commands are namespaced as `/symphony_main_<command>`.
...
- Node: `symphony`
- Database schema: `discord_music_symphony`
